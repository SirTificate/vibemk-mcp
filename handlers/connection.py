...
class ConnectionHandler(BaseHandler):
    ...
    async def _test_direct_url(self, test_url: Optional[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Best effort, for the same reason as api/client.py: a lazy socket read,
            # a body that need not be JSON, and a stdlib that raises a different type
            # per Python version when there is no body at all.
            try:
